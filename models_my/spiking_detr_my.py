...
from util_my import box_ops_my
from util_my.misc_my import (NestedTensor, nested_tensor_from_tensor_list, accuracy, get_world_size,
                             is_dist_avail_and_initialized)
from models_my.backbone_my import build_backbone
from models_my.matcher_my import build_matcher
from models_my.spikingtransformer_my import build_transformer
...
class MLP(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, output_dim, num_layers):
        ...
        self.layers = nn.ModuleList([layer.Linear(n, k) for n, k in zip([input_dim] + h, h + [output_dim])])
        self.neurons = nn.ModuleList([neuron.ParametricLIFNode() for _ in range(num_layers - 1)])
    ...
    def forward(self, x):
        ...
        for i, layer_ in enumerate(self.layers):
            x = self.neurons[i](layer_(x)) if i < self.num_layers - 1 else layer_(x)
        return x


class SpikingDETR(nn.Module):
    """ This is the SpikingDETR module that performs object detection """
    ...
    def __init__(self, backbone, transformer, num_classes, num_queries, aux_loss=False):
        ...
        # 最后的分类头
        self.class_embed = nn.Sequential(neuron.ParametricLIFNode(),
                                         layer.Linear(hidden_dim, num_classes + 1)
                                         )
        # 最后的bbox的头
        # MLP的输出output_dim 是4，是bbox的四个值
        self.bbox_embed = nn.Sequential(neuron.ParametricLIFNode(),
                                        MLP(hidden_dim, hidden_dim, 4, 3)
                                        )

        self.query_embed = nn.Embedding(num_queries, hidden_dim)
        # 1x1 衔接 backbone和transformer
        self.input_proj = layer.Conv2d(backbone.num_channels, hidden_dim, kernel_size=(1, 1))
        # backbone中的Joiner
    ...
    def forward(self, samples: NestedTensor):
        """The forward expects a NestedTensor, which consists of:
               - samples.tensor: batched images, of shape [T, batch_size x 3 x H x W]
               - samples.mask: a binary mask of shape [T, batch_size x H x W], containing 1 on padded pixels
        ...
        # input_proj [T,bs,2048,h,w] --input_proj--> [T,bs,256,h,w]
        ...
        # outpus_class (6,t,bs,100,92)
        # 类别的输出
        outputs_class = self.class_embed(hs).mean(dim=1)
        # [6,t,bs,100,4]
        # bbox的输出
        # 这里的bbox的值都是相对于图片的hw的因此都是小于1的，因此使用sigmoid将值约束在0-1
        outputs_coord = self.bbox_embed(hs).mean(dim=1).sigmoid()
        # 6层decoder,取最后一层的输出
        out = {'pred_logits': outputs_class[-1], 'pred_boxes': outputs_coord[-1]}

        # 计算辅助loss，计算前几层decoder输出的lossh
        if self.aux_loss:
...
class SetCriterion(nn.Module):
    """ This class computes the loss for spikinhDETR.
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes):
        ...
        # giou loss
        loss_giou = 1 - torch.diag(box_ops_my.generalized_box_iou(
            box_ops_my.box_cxcywh_to_xyxy(src_boxes),
            box_ops_my.box_cxcywh_to_xyxy(target_boxes)))
...
class PostProcess(nn.Module):
    ...
    def forward(self, outputs, target_sizes):
        ...
        boxes = box_ops_my.box_cxcywh_to_xyxy(out_bbox)
