    ...
    def forward(self, outputs, targets):
        ...
        Params:
            outputs: This is a dict that contains at least these entries:
                 "pred_logits": Tensor of dim [T, batch_size, num_queries, num_classes] with the classification logits
                 "pred_boxes": Tensor of dim [T, batch_size, num_queries, 4] with the predicted box coordinates
        ...
        # [T, batch_size * num_queries, 4]
        out_bbox = outputs["pred_boxes"].flatten(1, 2)
        ...
        # out_bbox is [T,bs*100,4] tgt_box is [all_img_gt_count,4]
        # Compute the L1 cost between boxes -> [T, bs*100, all_img_gt_count]
        # p=1 计算l1距离
        cost_bbox = torch.cdist(out_bbox, tgt_bbox.unsqueeze(0).repeat(T, 1, 1), p=1)  # cost_bbox 和 cost_class的维度是一样的
        ...
        cost_giou = -generalized_box_iou(box_cxcywh_to_xyxy(out_bbox.flatten(0, 1)),
                                         box_cxcywh_to_xyxy(tgt_bbox))
        _, all_img_gt_count = cost_giou.shape
        cost_giou = cost_giou.reshape(T, -1, all_img_gt_count)
        ...
        # [T, bs*100, all_img_gt_count] -> [T,bs,100,all_img_gt_count]
