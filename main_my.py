...
import datasets_my
import util_my.misc_my as utils
from datasets_my import build_dataset, get_coco_api_from_dataset
from engine_my import evaluate, train_one_epoch
from models_my import build_model
...
def main(args):
    ...
    if args.dataset_file == "coco_panoptic":
        ...
        coco_val = datasets_my.coco.build("val", args)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        lr_scheduler.step()
        if args.output_dir:
            # 在每个epoch后保存的文件都会刷新这个文件
            checkpoint_paths = [output_dir / 'checkpoint.pth']
            # extra checkpoint before LR drop and every 100 epochs
            if (epoch + 1) % args.lr_drop == 0 or (epoch + 1) % 100 == 0:
                # 每隔100个epoch,或者在lr_drop之前，保存一个
